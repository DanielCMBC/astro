...
This is the README that documented the original 3D prototype on the
`3D-test` branch, preserved verbatim when the scientific core landed.

The prototype itself is still in the repository root. It is the proof
that Python + OpenGL works for this project, and the roadmap is explicit
that it should be progressively replaced rather than extended - see
[`roadmap-status.md`](roadmap-status.md) for which of its known defects
are already fixed in the new core.
