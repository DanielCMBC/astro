...
| C3 distance and coordinate inspector | done - `coordinates/inspector.py`; `InspectorRow` / `CoordinateRow` / `NoteRow`, where a triplet cannot be built without naming its frame. Astropy owns ICRS/Galactic, the host-planet separation is the norm of the propagated float64 state with `r = a(1 - e cos E)` kept only as an independent check, and periapsis/apoapsis inherit input provenance. Adversarial tests pin that exaggeration, LOD, orbit sampling and camera placement cannot move a scientific value. See [`explorer-c.md`](explorer-c.md) |
